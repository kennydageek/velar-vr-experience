...
function DrivePhysics({
  ...
  useFrame((_, dt) => {
    ...
    const lateral =
      steer.current * Math.abs(velocity.current) * slipRef.current * 0.35;
    ...
    modelRef.current.position.x = THREE.MathUtils.clamp(
      modelRef.current.position.x,
      -420,
      420,
    );
    modelRef.current.position.z = THREE.MathUtils.clamp(
...
function CameraRig({
  ...
  const orbitRef = useRef<{ target: THREE.Vector3; update: () => void } | null>(
    null,
  );
...
function OpenLandscape() {
  const terrain = useMemo(() => {
    ...
    for (let i = 0; i < p.count; i++) {
      ...
      const h =
        Math.sin(x * 0.03) * 1.1 +
        Math.cos(y * 0.022) * 0.9 +
        Math.sin((x + y) * 0.014) * 1.4;
      p.setZ(i, h);
    }
...
export function DriveSimulator() {
  ...
  return (
    <section className="relative h-screen bg-[#2f2927] text-white">
      <Canvas
        dpr={[1, 1.5]}
        camera={{ position: [-3.1, 1.9, -3.1], fov: 55 }}
        shadows
      >
      ...
      <div className="absolute bottom-4 left-4 right-4 grid gap-2 md:grid-cols-3">
        ...
        <div className="rounded-2xl border border-white/20 bg-black/35 p-3 text-[11px] text-white/85">
          ...
          <p className="mb-1 mt-3 text-[10px] tracking-[0.2em] text-white/60">
            HANDLING
          </p>
          <div className="flex gap-1.5">
            {gripModes.map((g) => (
